...
for (seq_id in sequences) {
  ...
  if (hits_count >= 2) {
    ...
    pdf(file = taxonomic_file1, width=14, height=7)
    taxonomic_plot1
    dev.off()
    taxonomic_plot2 <- barplot(table(anno$source))
    taxonomic_file2 <- paste("Taxonomic_2_", seq_id, ".pdf", sep="")
    
    pdf(file = taxonomic_file2, width=14, height=7)
    taxonomic_plot2
    dev.off()
